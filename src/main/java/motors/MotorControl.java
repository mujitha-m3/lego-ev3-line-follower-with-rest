...
public class MotorControl implements Runnable {
	...
	/**
	 * Initializes a new instance of the MotorControl class, setting up the motors
	 * and storing the configuration.
	 * 
	 * @param config The configuration object containing operational parameters and
	 *               sensor thresholds.
	 */
	public MotorControl() {
	...
	/**
	 * The entry point for the MotorControl thread. Waits for initial calibration to
	 * complete, then enters a loop where it continuously checks sensor values and
	 * adjusts motor speeds accordingly to navigate the robot.
	 */
	@Override
	public void run() {
	...
	/**
	 * Implements a PID control strategy to adjust the robot's movement based on
	 * sensor feedback. This method calculates the necessary adjustments to the
	 * motor speeds for turning and straight movement, aiming to maintain the
	 * desired trajectory.
	 */
	public void turn() {
		...
		/*
		 * The turn method utilizes a PID controller to dynamically adjust the robot's
		 * heading based on the difference (error) between the desired trajectory and
		 * the current sensor reading. This method adapts the motor speeds to correct
		 * for deviations, such as veering off a path or approaching an obstacle,
		 * ensuring smooth and effective navigation.
		 */
		error = config.SENSOR_VALUE - config.THRESHOLD;
		integral += error;
		derivative = error - lastError;
		lastError = error;
		PIDsum = error * config.PROPORTIONAL_GAIN + integral * config.INTEGRAL_GAIN
				+ derivative * config.DERIVATIVE_GAIN;
	...
	/*
	 * The handleObjectAvoidance method is called when an obstacle is detected in
	 * the robot's path. It temporarily overrides the normal navigation to execute a
	 * maneuver designed to avoid the obstacle, involving stopping, reversing,
	 * turning, and then proceeding forward. After object has been passed the robot
	 * automatically starts looking for the line again.
	 */
	private void handleObjectAvoidance() {
		...
		/**
		 * Turn to avoid the object, by stopping the left motor and setting the right
	...
	/**
	 * Adjusts the speed and direction of the motors based on the given parameters.
	 * This method can be used to set the motors for forward movement, reverse, or
	 * turning, depending on the speeds and the reverse flag.
	 * 
	 * @param leftSpeed  The speed for the left motor.
	 * @param rightSpeed The speed for the right motor.
	 * @param reverse    A flag indicating whether the motors should be set to
	 *                   reverse.
	 */
	...
	/**
	 * Stops both motors immediately. This is used for emergency stopping or as part
	...
	/**
	 * Closes the motor ports, effectively disabling the motors. This method is
	 * called before shutting down the program to ensure a graceful termination of
	 * motor control.
	 */
